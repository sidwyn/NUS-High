...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
	NSArray *oneTouch = [touches allObjects];
	UITouch *onlyFirst = [oneTouch objectAtIndex:0];
	CGPoint veryFirstPoint = [onlyFirst locationInView:self.view];
	if (veryFirstPoint.y>460 && veryFirstPoint.y < 480){
		for (UIView *view in ar_overlayView.subviews) {
			[view removeFromSuperview];
		}
		[self.cameraController setCameraOverlayView:nil];
		[self dismissModalViewControllerAnimated:YES];
		[self performSelector:@selector(handleTimer) withObject:self afterDelay:2];
		NSLog(@"Calling handleTimer");
	}
}

-(void)handleTimer{
	if([self.delegate respondsToSelector:@selector(isARDone)]){
		[self.delegate isARDone];
		NSLog(@"Calling isARDone");
	}
}
